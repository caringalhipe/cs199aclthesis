...
def process_input(data, highlight_edges):
    ...
    fig, ax = plt.subplots()
    nx.draw(G, pos, with_labels=True, node_size=1000, node_color=color_map, node_shape='s', edge_color=edge_colors, ax=ax)
...
def submit_input():
    ...
    # Prompt user for edges to highlight after entering linear orders
    ...
    display_graph(data, highlight_edges)

# Display graph based on input data
def display_graph(data, highlight_edges):
    global canvas
    ...
    fig = process_input(data, highlight_edges)
